...
export class HistoryManager {
  ...
  static getDuration = (serverConfig: ComputedServerConfig, urlParams?: URLSearchParams): number | undefined => {
    const duration = HistoryManager.getNumericParam(URLParam.DURATION, urlParams);
    if (duration) {
      return toValidDuration(serverConfig, Number(duration));
    }
    return undefined;
  };

  static getRangeDuration = (serverConfig: ComputedServerConfig, urlParams?: URLSearchParams): number | undefined => {
    const rangeDuration = HistoryManager.getNumericParam(URLParam.RANGE_DURATION, urlParams);
    if (rangeDuration) {
      return toValidDuration(serverConfig, Number(rangeDuration));
    }
    return undefined;
  };
