...
import IstioConfigCard from '../../components/IstioConfig/IstioConfigCard';
...
export class WorkloadInfo extends React.Component<WorkloadInfoProps, WorkloadInfoState> {
  ...
  private fetchBackend = () => {
    ...
    getIstioConfig(this.props.namespace, workloadIstioResources, true, '', workloadSelector).then(results => {
  ...
  private workloadValidations(workload: Workload): Validations {
    ...
    const isWaypoint = workload.labels && workload.labels['gateway.istio.io/managed'] === 'istio.io-mesh-controller';
    ...
    if (workload.pods.length > 0) {
      ...
      workload.pods.forEach(pod => {
        ...
        if (!isIstioNamespace(this.props.serverConfig, this.props.namespace)) {
          if (!isWaypoint) {
