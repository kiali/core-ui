...
export type WorkloadPodLogsProps = {
...
const logsHeight = (showToolbar: boolean, fullscreen: boolean, showMaxLinesWarning: boolean) => {
  ...
  return {
    height: fullscreen
      ...
      : `calc(500px + ${toolbarHeight} - ${maxLinesWarningHeight})`
  };
};
...
export class WorkloadPodLogs extends React.Component<WorkloadPodLogsProps, WorkloadPodLogsState> {
    ...
    const logDropDowns = Object.keys(LogLevel).map(level => {
      return (
        <DropdownItem
          ...
          isDisabled={this.props.serverConfig.deployment.viewOnlyMode}
        >
          {level}
        </DropdownItem>
      );
  ...
  private setLogLevel = (level: LogLevel) => {
    ...
    setPodEnvoyProxyLogLevel(this.props.namespace, pod.name, level)
      .then(_resp => {
        console.log(`Successfully updated proxy log level to '${level}' for pod: ${pod.name}`);
