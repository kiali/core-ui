...
import {
  ...
  TimeInMilliseconds,
  getPodEnvoyProxy,
  getPodEnvoyProxyResourceEntries,
...
import { SummaryTableBuilder } from './tables/BaseTableBuilder';
...
const workloadTabName = 'tab';
const workloadDefaultTab = 'info';
...
export class EnvoyDetails extends React.Component<EnvoyDetailsProps, EnvoyDetailsState> {
  ...
  fetchEnvoyProxyResourceEntries = (resource: string) => {
    getPodEnvoyProxyResourceEntries(this.props.namespace, this.state.pod.name, resource)
  ...
  fetchEnvoyProxy = () => {
    getPodEnvoyProxy(this.props.namespace, this.state.pod.name)
      .then(resultEnvoyProxy => {
        this.setState({
          config: resultEnvoyProxy.data,
          fetch: false
  ...
  render() {
    ...
    const height = 400;
    const app = this.props.workload.labels[this.props.serverConfig.istioLabels.appLabelName];
    const version = this.props.workload.labels[this.props.serverConfig.istioLabels.versionLabelName];
