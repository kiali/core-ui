import { MetricsSettings, LabelsSettings, LabelSettings, Quantiles } from '../MetricsOptions/MetricsSettings';
import {
  boundsToDuration,
  guardTimeRange,
  TimeRange,
  ...
  metrics.forEach(m => {
    Object.keys(m.labels).forEach(k => {
      ...
      if (agg) {
        ...
        if (!Object.prototype.hasOwnProperty.call(lblObj.values, value)) {
...
export const timeRangeToOptions = (serverConfig: ComputedServerConfig, range: TimeRange, opts: MetricsQuery) => {
  ...
  const intervalOpts = computePrometheusRateParams(serverConfig, opts.duration);
