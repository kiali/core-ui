...
export class CustomMetrics extends React.Component<CustomMetricsProps, MetricsState> {
  ...
  private initOptions(settings: MetricsSettings): DashboardQuery {
    const filters = `${this.props.serverConfig.istioLabels.appLabelName}:${this.props.app}`;
    const options: DashboardQuery = this.props.version
      ? {
          labelsFilters: `${filters},${this.props.serverConfig.istioLabels.versionLabelName}:${this.props.version}`
        }
      : {
  ...
  componentDidUpdate(prevProps: CustomMetricsProps) {
    if (
  ...
  private fetchMetrics = () => {
    ...
    getCustomDashboard(this.props.namespace, this.props.template, this.options)
      .then(response => {
