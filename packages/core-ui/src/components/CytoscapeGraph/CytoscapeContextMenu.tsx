...
export class CytoscapeContextMenuWrapper extends React.PureComponent<Props> {
  ...
  connectCy(serverConfig: ComputedServerConfig, cy: Cy.Core) {
    cy.on('cxttapstart', 'node,edge', (event: Cy.EventObject) => {
      event.preventDefault();
      if (event.target) {
        this.handleContextMenu(serverConfig, event.target, false);
      }
      return false;
    });
  }
  ...
  handleContextMenu(serverConfig: ComputedServerConfig, elem: Cy.NodeSingular | Cy.EdgeSingular, isHover: boolean) {
    const contextMenuType = elem.isNode() ? this.props.contextMenuNodeComponent : this.props.contextMenuEdgeComponent;

    if (contextMenuType) {
      this.makeContextMenu(serverConfig, contextMenuType, elem, isHover, elem.isNode());
    }
  }
  ...
  private makeContextMenu(
    ...
    if (!isHover && (target.isEdge() || getOptions(serverConfig, { ...target.data() }).length === 0)) {
      return;
    }
