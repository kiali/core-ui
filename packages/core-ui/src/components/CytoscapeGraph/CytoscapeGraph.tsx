...
import { refreshLabels, runLayout } from './graphs/GraphStyles';
...
export default class CytoscapeGraph extends React.Component<CytoscapeGraphProps, CytoscapeGraphState> {
  ...
  constructor(props: CytoscapeGraphProps) {
    ...
    const settings = props.serverConfig.kialiFeatureFlags.uiDefaults.graph.settings;
      ...
      <div id="cytoscape-graph" className={this.props.containerClassName} ref={this.cytoscapeGraphRef}>
        ...
        >
          ...
          <CytoscapeReactWrapper
            ref={e => this.setCytoscapeReactWrapperRef(e)}
  ...
  private cyInitialization(cy: Cy.Core) {
    ...
    this.contextMenuRef!.current!.connectCy(this.props.serverConfig, this.cy);
    ...
    cy.on('mouseover', 'node,edge', (evt: Cy.EventObject) => {
      ...
      CytoscapeGraph.mouseInTimeout = setTimeout(() => {
        ...
        if (noLabels) {
          this.contextMenuRef!.current!.handleContextMenu(this.props.serverConfig, cytoscapeEvent.summaryTarget, true);
        }
    ...
    cy.on('zoom', (evt: Cy.EventObject) => {
      ...
      if (thresholdCrossed) {
        refreshLabels(this.props.serverConfig, cy, false);
      }
    });
  ...
  private processGraphUpdate(cy: Cy.Core, runLayoutFlag: boolean, newLayout: boolean): Promise<void> {
    ...
    const globalScratchData: CytoscapeGlobalScratchData = {
      ...
      homeCluster: this.props.serverConfig?.clusterInfo?.name || CLUSTER_DEFAULT,
    ...
    assignEdgeHealth(this.props.serverConfig, cy);
    ...
    if (runLayoutFlag) {
      return new Promise((resolve, _reject) => {
        runLayout(this.props.serverConfig, cy, this.props.layout, this.props.namespaceLayout).then(_response => {
          this.finishGraphUpdate(cy, isTheGraphSelected, runLayoutFlag);
          resolve();
        });
      });
    } else {
  ...
  private selectTarget = (target?: Cy.NodeSingular | Cy.EdgeSingular | Cy.Core) => {
    if (this.cy) {
      ...
      if (target && !isCore(target)) {
        (target as Cy.NodeSingular | Cy.EdgeSingular | Cy.Core).selectify().select().unselectify();
      }
    }
  };
