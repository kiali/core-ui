...
import { ComputedServerConfig } from '@kiali/types';
...
export class CytoscapeReactWrapper extends React.Component<CytoscapeReactWrapperProps, CytoscapeReactWrapperState> {
  ...
  private build() {
    ...
    const opts = {
      ...
      style: GraphStyles.styles(this.props.serverConfig),
      ...GraphStyles.options()
    };

    this.cy = cytoscape(opts);
    // Using an extension
    (this.cy as any).nodeHtmlLabel(GraphStyles.htmlNodeLabels(this.props.serverConfig, this.cy));
  }
