...
import { kialiBadge, PFBadgeType, PFBadges } from '../../Pf/PfBadges';
...
export const refreshLabels = (serverConfig: ComputedServerConfig, cy: Cy.Core, force: boolean) => {
  ...
  cy.edges().each(e => {
    e.data('label', GraphStyles.getEdgeLabel(serverConfig, e, e.selected()));
  });
...
export const runLayout = (
  ...
  refreshLabels(serverConfig, cy, true);

  const layoutOptions = getLayout(layout);
...
export class GraphStyles {
  ...
  static getNodeLabel(serverConfig: ComputedServerConfig, ele: Cy.NodeSingular) {
    ...
    if (isBox) {
      ...
      const pfBadgeStyle = pfBadge ? style(pfBadge.style as NestedCSSProperties) : '';
  ...
  static htmlNodeLabels(serverConfig: ComputedServerConfig, cy: Cy.Core) {
    return [
      {
        ...
        halignBox: 'center',
        valignBox: 'bottom',
        tpl: (data: any) => this.getNodeLabel(serverConfig, cy.$id(data.id))
      },
      {
  ...
  static getEdgeLabel = (serverConfig: ComputedServerConfig, ele: Cy.EdgeSingular, isVerbose?: boolean): string => {
    const settings = serverConfig.kialiFeatureFlags.uiDefaults.graph.settings;
    const zoom = ele.cy().zoom();
    const cyGlobal = GraphStyles.getCyGlobalData(ele);
  ...
  static styles(serverConfig: ComputedServerConfig): Cy.Stylesheet[] {
      ...
      },
      {
        ...
        css: {
          ...
          label: (ele: Cy.EdgeSingular) => {
            return GraphStyles.getEdgeLabel(serverConfig, ele);
          },
